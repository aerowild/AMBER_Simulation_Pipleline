...
use warnings;
use diagnostics;
unlink 'protein_denaturation.log';
unlink 'setup_error.log';
open (STDOUT,  "| tee -ai protein_denaturation.log");
open (STDERR,  "| tee -ai setup_error.log");
#open(STDERR, '>', 'protein_denaturation_error.log') or die "Can't open log";
#______________________________________________________________________________________________________________
#______________________________________________________________________________________________________________
# DO NOT MODIFY EVEN A CHARACTER HERE IN THIS SCRIPT, ITS NOT INTELLIGENT. 
# DO NOT DISTURB A LINKED DIRECTORY NAMED AS '/protein_denaturation_FILES/raw'. IT CONTAINS THE BASIC FILES REQUIRED TO GENERATE MODIFY FILES
...
system ("cp -R ~/Shashank_scripts/protein_denaturation_FILES/raw raw");
mkdir 'topology';
...
#Modifying run_denaturation.pl file
#$i_runMD = 'run_denaturation.pl'; open(READRUNMD, "$i_runMD");
...
system("cp raw/progressive_heating_creator.pl productionMD/progressive_heating_creator.pl");

system("cp raw/run_denaturation.pl run_denaturation.pl");
system("rm -R raw");
system('clear');

#mkdir 'productionMD/analysis';
#system('cp raw/ptraj.in productionMD/analysis/ptraj.in');


#Creating windows for progressive heating
chdir 'productionMD';
...
print 'Whoopee...Your system is ready to run protein denaturaion MD', "\n", 'Now I am ready to pull the trigger, I mean to run run_denaturation.pl...shall I?'. "\n";
print "Type run to execute run_denaturation.pl, else hit enter if you want to run it manually\n";
$runMD = <>;
if ($runMD =~ q/run/ or $runMD =~ q/RUN/) {system('perl run_denaturation.pl');}else {print "Very well, looks like you plan to revolutionise the MD...Good Luck\n Suggestion: DO NOT RUN ME AGAIN IF EVERYTHING IS READY, I WILL CLEAR THE HISTORY AND START NEW BEGENING. JUST RUN run_denaturation.pl\n";}
